...
private:
	...
	// for thread-safety
	mutex m;
	condition_variable data_available; // for wait by the pop, signaled by push func
	condition_variable slot_available; // waited on by the push, signaled by pop func
...
public:
	...
	void push(char* data, int len){
		...
		// 3. Then push the vector at the end of the queue, watch out for racecond.
		q.push(d);
		l.unlock();
	...
	int pop(char* buf, int bufcap){
		...
		// 4. Wake up any potentially push() function
